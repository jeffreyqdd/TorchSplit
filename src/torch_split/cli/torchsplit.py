...
import torch_split.core.solver as solver
...
def assert_model_cache(model_spec: ModelSpec, no_cache: bool = False):
    ...
    profiling_files = {
        bs: annotation_directory / f"profiling_batchsize_{bs}.json"
        for bs in model_spec.split_client.batch_sizes()
    }

    device_files = {
        bs: annotation_directory / f"device_batchsize_{bs}.json"
        for bs in model_spec.split_client.batch_sizes()
    }
    ...
    files_missing = any(
        not x.exists() or not y.exists()
        for x, y in zip(profiling_files.values(), device_files.values())
    )
    ...
    if require_update or no_cache or files_missing:
        ...
        for bs in model_spec.split_client.batch_sizes():
            ...
            warmup_rounds, iterations, generator = (
                model_spec.split_client.get_benchmarks(bs)
            )

            args, kwargs = next(generator)
            gm = utils.capture_graph(model_spec.split_client, *args, **kwargs)
    ...
    for bs in model_spec.split_client.batch_sizes():
        with open(profiling_files[bs], "r") as file:
            profiling_data = json.load(file)

        with open(device_files[bs], "r") as file:
...
def main():
    ...
    for split_client_spec in program_args.model:
        ...
        try:
            ...
            logger.exception(
                "failed to load SplitClient '%s:%s'", module_path, class_name
            )
            sys.exit(1)
    ...
    partitions: dict[str, tuple[ModelSpec, core.PartitionProvider]] = {}
    for model_name, model_spec in target_models.items():
        ...
        tg = core.TorchGraph.from_fx_graph(gm, label=model_name)
        ...
        partitions[model_name] = (model_spec, core.PartitionProvider(tg))
    ...
    if program_args.dataflow:
        for partition in partitions.values():
            ...
            partition[1].visualize_dominance(output_dir / "visualizations")
